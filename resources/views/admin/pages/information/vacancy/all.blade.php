...
@section('page-title', 'All Vacancy List Information ')
...
<div class="card">
	<div class="card-header">
		<h2 style="text-align:center">All Vacancy List Information</h2>
			...
			<table id="multi-table" class="table table-striped table-bordered nowrap">
				...
				@foreach($vacancy as $data)
					...
					<tr>
						...
						<td>{{ $data->emptySeats }}</td>
						<td>{{ $data->details }}</td>
						<td><p class="text-center"><img class="img-circle imageSize img-responsive" src="{{url($data->image ? $data->image : '')}}"  alt="Image"></p>
						</td>

						<td>
                            <a href="{{url('/vacancy/'.$data->id.'/edit')}}" class="btn hor-grd btn-grd-success">Edit</a>

                            <a  href="{{url('/vacancy/delete/'.$data->id)}}" class="btn hor-grd btn-grd-danger">Delete</a>                               

			...
			<table id="multi-table" class="table table-striped table-bordered nowrap">
				...
				@foreach($event as $data)
					...
					<tr>
						...
						<td>{{ $data->event->category }}</td>
						<td><p class="text-center"><img class="img-circle imageSize img-responsive" src="{{url($data->image ? $data->image : '')}}"  alt="Image"></p>
						</td>

						<td>
                            <a href="{{url('/eventInfo/'.$data->id.'/edit')}}" class="btn hor-grd btn-grd-success">Edit</a>

                            <a  href="{{url('/eventInfo/delete/'.$data->id)}}" class="btn hor-grd btn-grd-danger">Delete</a>
